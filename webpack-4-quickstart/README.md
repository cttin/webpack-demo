...
1. 初始化参数（entry-option）：在加载插件之前，webpack从配置文件和 Shell 语句中读取与合并参数，得出最终的参数；
2. 开始编译（run）：webpack实际的入口是compiler中的run方法，run 一旦执行后，就开始了编译和构建流程。这里用上一步得到的参数初始化 Compiler 对象，加载所有配置的插件，执行对象的 run 方法开始执行编译；
...
10. 输出资源（seal）：在所有模块及其依赖模块 build 完成后，webpack 会监听 seal 事件调用各插件对构建后的结果进行封装，要逐次对每个 module 和 chunk 进行整理，生成编译后的源码，合并，拆分，生成 hash 。即根据入口和模块之间的依赖关系，组装成一个个包含多个模块的 Chunk，再把每个 Chunk 转换成一个单独的文件加入到输出列表，这步是可以修改输出内容的最后机会，比如合并，抽取公共模块、加hash、压缩代码，插件 UglifyJsPlugin 就放在这个阶段；
11. bootstrap： 生成启动代码；
